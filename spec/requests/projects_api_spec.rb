...
RSpec.describe "Projects API", type: :request do
  ...
  context "#destroy" do
    ...
    it "not found a project" do
      user = FactoryBot.create(:user)
      project = FactoryBot.create(:project, owner: user)

      expect {
        delete api_project_path(200), params: {
          user_email: user.email,
          user_token: user.authentication_token
        }
      ...
      error_message = JSON.parse(response.body)["errors"].values
      expect(error_message).to include("対象がないです")
    end
  end
end
